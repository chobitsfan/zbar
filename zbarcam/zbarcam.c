...
int send_curl(char *httpstr)
{
  ...
  if(curl) {
    /* First set the URL that is about to receive our POST. This URL can
       ...
       data. */
    curl_easy_setopt(curl, CURLOPT_URL, "http://60.250.195.11/drone/droneReport.php?tokenID=1");
...
static void data_handler (zbar_image_t *img, const void *userdata)
{
    ...
    for(; sym; sym = zbar_symbol_next(sym)) {
        ...
        if(!format) {
            ...
            char httpstr[256];
            memset(httpstr,0x0,256);
            memcpy(httpstr, zbar_symbol_get_data(sym),zbar_symbol_get_data_length(sym));
            //send_curl(httpstr);
            
            printf("%s: %s", zbar_get_symbol_name(type),httpstr);
